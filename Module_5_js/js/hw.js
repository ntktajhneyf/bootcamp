...
const getUsersByGender = (arr, gender) => { 
  ...
  const name = arr.filter(arr => arr.gender === gender  );
  const newName = name.map(name => name.name)
return newName}
...
const getInactiveUsers = arr => {
  const isActive = arr.filter(arr=> arr.isActive === false)
  const name = isActive.map(isActive => isActive.name);
  return name;
};
 ...
 */
const getUsersWithAge = (arr, min, max) => arr.filter(arr => arr.age < max & arr.age > min);
...
const getUsersByFriend = (arr, name) => {
  ...
  const mass = arr.filter(arr => arr.friends.includes(name))
 const massNew = mass.map(mass => mass.name)
